...
inline std::string build_usage_text() {
  ...
  out << "Usage:\n"
      ...
      << "  --progress-interval. Use --tokenizer byte|bpe|pairs to choose the token stream.\n"
      << "  When using --tokenizer bpe or pairs, pass --tokenizer-vocab N to control the learned vocabulary size.\n"
      << "  Use --val-input for an explicit validation file, --split-ratio for the\n"
      << "  automatic split, or --no-val-split to train and validate on all input.\n"
